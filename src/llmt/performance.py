...
def binary_performance(y_true: List[int], y_pred: List[int], decimals: int=4) -> dict:
    ...
    performance_dict = {'p': y_true.count(1), # Number of positive samples in true data
                        ...
                        'fp': int(fp),
                        'fn': int(fn),
                        'recall': round(float(tp / (tp + fn)), decimals), # Sensitivity true positive rate
                        'precision': round(float(tp / (tp + fp)), decimals), # Fraction of correctly identified positive samples
